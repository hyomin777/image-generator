...
def evaluate_similarity_matrix(tag_encoder, image_encoder, tokenizer, dataset, device, num_samples=32, save_path='output/similarity_matrix.png'):
    ...
    with torch.no_grad():
        ...
        tokenized = tokenizer(texts, padding=True, truncation=True, max_length=64, return_tensors='pt').to(device)
...
def retrieve_images_by_text(tag_encoder, image_encoder, tokenizer, dataset, text_query, device, top_k=5):
    ...
    with torch.no_grad():
        ...
        tokenized = tokenizer([text_query], padding=True, truncation=True, max_length=64, return_tensors='pt').to(device)
...
def plot_positive_negative_distribution(tag_encoder, image_encoder, tokenizer, dataset, device, num_samples=128, save_path='output/pos_neg_hist.png'):
    ...
    for i in range(min(num_samples, len(dataset) - 1)):
        ...
        with torch.no_grad():
            ...
            pos_tok = tokenizer([text], padding=True, truncation=True, max_length=64, return_tensors='pt').to(device)
            neg_tok = tokenizer([neg_text], padding=True, truncation=True, max_length=64, return_tensors='pt').to(device)
