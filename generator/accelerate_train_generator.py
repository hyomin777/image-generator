...
def train_generator(args):
    ...
    start_epoch, best_loss, image_generator.text_encoder = load_checkpoint(image_generator.text_encoder, device, Path(args.output_dir), f'generator_text_encoder_2')

    vae = image_generator.vae
    text_encoder = image_generator.text_encoder
    vae.requires_grad_(False)
    delattr(image_generator, 'vae')
    delattr(image_generator, 'text_encoder')
    ...
    image_generator, optimizer, dataloader = accelerator.prepare(
        image_generator, optimizer, dataloader
    )
    image_generator.vae = vae.to(device)
    image_generator.text_encoder = text_encoder.to(device)

    global_step = 1
    best_loss = 1
    print(type(image_generator.unet.time_embedding.linear_1.weight))
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in progress_bar:
            ...
            accelerator.clip_grad_norm_(image_generator.parameters(), max_norm=0.5)
            optimizer.step()
