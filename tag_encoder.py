...
class TagEncoder(nn.Module):
    ...
    def forward(self, tags):
        inputs = self.tokenizer(tags, padding=True, truncation=True, return_tensors='pt').to(self.bert.device)
        outputs = self.bert(**inputs).last_hidden_state[:, 0, :] # (batch_size, hidden_dim) CLS token for each sample
